...
def main(args):
    params = utils.get_charuco_board_parameters()
    frame_width = args.frame_width
    frame_height = args.frame_height

    # Set up webcam
    cap = utils.get_video_cap(frame_width, frame_height, args.camera_id)

    # Set up aruco dict and board
    aruco_dict = cv2.aruco.Dictionary_get(params['dict_id'])
    board = cv2.aruco.CharucoBoard_create(params['squares_x'], params['squares_y'], params['square_length'], params['marker_length'], aruco_dict)
    ...
    rep_error_aruco, camera_matrix, dist_coeffs, _, _ = cv2.aruco.calibrateCameraAruco(all_corners_concatenated, all_ids_concatenated, marker_counter_per_frame, board, (frame_width, frame_height), None, None)

    # Charuco calibration using previous camera parameters
    all_charuco_corners = []
    all_charuco_ids = []
    ...
    if len(all_charuco_corners) < 4:
        ...
        sys.exit()
    rep_error, camera_matrix, dist_coeffs, _, _ = cv2.aruco.calibrateCameraCharuco(all_charuco_corners, all_charuco_ids, board, (frame_width, frame_height), camera_matrix, dist_coeffs)

    print('Rep Error:', rep_error)
    print('Rep Error Aruco:', rep_error_aruco)

    # Save camera parameters
    camera_params_file_path = utils.get_camera_params_file_path(camera_name=args.camera_name)
    camera_params_dir = Path(camera_params_file_path).parent
    if not camera_params_dir.exists():
        camera_params_dir.mkdir()
    fs = cv2.FileStorage(camera_params_file_path, cv2.FILE_STORAGE_WRITE)
    fs.write('image_width', frame_width)
    fs.write('image_height', frame_height)
